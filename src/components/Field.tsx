...
function Field({
  ...
  return (
    <div className="w-full flex flex-wrap">
      {/* flex-wrap causes cells to flow into rows automatically based on their percentage width */}
      {Object.entries(cells).map(
        (
          [cellId, cellData] // Cells are iterated in insertion order, which matches the y-then-x loop in emptyCells()
        ) => (
          <Cell
